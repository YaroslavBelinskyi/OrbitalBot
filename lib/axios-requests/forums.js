...
async function getRandomPictureWowlol() {
    try {
        const picturesPage = await axios.get('https://wowlol.ru/random');
        const { data } = picturesPage;
        const dom = new JSDOM(data);
        const picture = dom.window.document.querySelector('.block:nth-of-type(2) .binner img').src;
        return picture;
    } catch (error) {
        console.log(error);
    }
