...
async function getArticlesPlayua() {
    try {
        ...
        const { data } = articlesPage;
        const dom = new JSDOM(data);
        // const event = await simulant(dom.window, 'click');
        // const element = dom.window.document.querySelector('#load-more-posts-btn');
        // await simulant.fire(element, event);
...
async function getRandomJoke() {
    try {
        const jokesPage = await axios.get('https://shytok.net/sluchainye-anekdoty.html');
        const { data } = jokesPage;
        const dom = new JSDOM(data);
        const joke = dom.window.document.querySelector('#anek:first-of-type #text');
        return joke;
    } catch (error) {
        console.log(error);
    }
